...
/**
 * Computes the Euclidean distance observation from LIDAR and Map's landmark.
 * @param (x1,y1) x and y coordinates of observation
 * @param (x2,y2) x and y coordinates of Map's landmark
 * @output Euclidean distance between two 2D points
 */
inline double dist(LandmarkObs observation, LandmarkObs landmark) {
  return sqrt((landmark.x - observation.x) * (landmark.x - observation.x) + (landmark.y - observation.y) * (landmark.y - observation.y));
}
